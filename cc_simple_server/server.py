...
async def create_task(task_data: TaskCreate):
    ...
    conn.close()
    return TaskRead(id=task[0], title=task[1], description=task[2], completed=bool(task[3]))
...
async def get_tasks():
    ...
    conn.close()
    return [TaskRead(id=task[0], title=task[1], description=task[2], completed=bool(task[3])) for task in tasks]
